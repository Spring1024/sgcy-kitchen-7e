      ...
      <div className="flex">
        ...
        <main className="flex-1 px-4 pb-32 pt-4 min-w-0">
          ...
          {searchQuery.trim() && filteredItems.length === 0 ? <div className="flex flex-col items-center justify-center pt-20 text-center">
              ...
              {activeCategoryData && filteredItems.map((item, index) => <MenuItem key={item.id} item={item} quantity={getItemQuantity(item.id)} onAdd={() => addToCart(item)} onRemove={() => removeFromCart(item.id)} onDetail={() => handleDetail(item.id)} index={index} />)}
            </div>}
        </main>
      </div>

      {/* Floating Cart Button */}
      {totalCount > 0 && <div className="fixed bottom-20 left-4 right-4 z-40">
